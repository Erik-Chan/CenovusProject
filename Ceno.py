...
#This is used to generate some OU processes and plot them to get a sense for what the data looks like that we
#are inputting into the model. By default, when toggle = 0 this does not do anything.
toggle = 0
if toggle:
...
if MSE:
    ...
    if test:
        # localPrices = my_ou(x0=30, k=2, theta=30, sigma=5, paths=3, steps=1000)(t)
        localPrices1 = my_ou(x0=20, k=2, theta=15, sigma=10, paths=1, steps=1000)(t)
        localPrices2 = my_ou(x0=30, k=2, theta=15, sigma=10, paths=1, steps=1000)(t)
        localPrices3 = my_ou(x0=40, k=2, theta=15, sigma=10, paths=1, steps=1000)(t)
    ...
    for s in range(len(verts)):
        til_W_t = til_W_st[s]
        for _t in range(len(t)):
            model += til_W_t <= psi_t[_t]*M

    # Constraint (20e)
    model += xsum(psi_t) <= np.floor(beta * T)

    #Just a print statement to debug
    print('The value of np.floor(beta * T) is:', np.floor(beta * T))
